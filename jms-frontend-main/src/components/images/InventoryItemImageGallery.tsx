...
import { Upload, Star, Trash2, Eye, AlertCircle, CheckCircle, Camera, X } from 'lucide-react';
import { ConfirmDialog } from '../ui/ConfirmDialog';

interface InventoryItemImageGalleryProps {
  inventoryItemId: string;
  itemCode: string;
}

export const InventoryItemImageGallery: React.FC<InventoryItemImageGalleryProps> = ({ inventoryItemId, itemCode }) => {
  ...
  const canCreate = hasPermission('inventory_item_image.create');
  const canUpdate = hasPermission('inventory_item_image.update');
  const canDelete = hasPermission('inventory_item_image.delete');
  ...
  const fetchImages = async () => {
    ...
    setErrorMsg(null);
    try {
      const res = await imagesApi.listInventoryItemImages(inventoryItemId);
      if (res?.success || Array.isArray(res?.data)) {
        setImages(res.data || []);
      }
    } catch (err: any) {
      setErrorMsg(err?.response?.data?.message || 'Failed to load physical item photographs.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    setSuccessMsg(null);
    try {
      await imagesApi.uploadInventoryItemImage(inventoryItemId, selectedFile, altText, isPrimary);
      setSuccessMsg('Physical jewellery photograph uploaded successfully.');
      setSelectedFile(null);
      setAltText('');
      setIsPrimary(false);
      fetchImages();
    } catch (err: any) {
      setErrorMsg(err?.response?.data?.message || 'Failed to upload physical item photograph.');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSetPrimary = async (imageId: string) => {
    try {
      await imagesApi.updateInventoryItemImage(inventoryItemId, imageId, { isPrimary: true });
      setSuccessMsg('Primary physical photograph updated.');
      fetchImages();
    } catch (err: any) {
      setErrorMsg(err?.response?.data?.message || 'Failed to update primary photograph.');
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await imagesApi.deleteInventoryItemImage(inventoryItemId, deleteTarget);
      setSuccessMsg('Photograph deleted successfully.');
      fetchImages();
    } catch (err: any) {
      setErrorMsg(err?.response?.data?.message || 'Failed to delete photograph.');
    } finally {
    ...
    <div style={{ marginTop: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        <div>
          <h4 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#18181B', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Camera size={18} color="#C6A15B" /> Physical Jewellery Photographs ({images.length})
          </h4>
          <p style={{ fontSize: '0.78rem', color: '#64748B' }}>
            Actual physical item photographs for tagged inventory piece <strong>{itemCode}</strong>.
          </p>
        </div>
      </div>
      ...
      {canCreate && (
        <div className="glass-card" style={{ padding: '16px', background: '#F8FAFC', borderRadius: '10px', marginBottom: '20px', border: '1px dashed #CBD5E1' }}>
          <h5 style={{ fontSize: '0.88rem', fontWeight: 700, marginBottom: '10px', color: '#18181B' }}>Upload Physical Jewellery Photograph</h5>
          <form onSubmit={handleUpload} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto auto', gap: '12px', alignItems: 'end' }}>
            <div>
              <label className="form-label" style={{ fontSize: '0.75rem' }}>Select Photo (JPEG, PNG, WEBP ≤ 5MB)</label>
            ...
            <div>
              <label className="form-label" style={{ fontSize: '0.75rem' }}>Alt Text / Hallmark Note</label>
              <input type="text" value={altText} onChange={(e) => setAltText(e.target.value)} placeholder="BIS Hallmark stamp, diamond clarity..." className="form-input" style={{ padding: '7px 10px', fontSize: '0.82rem' }} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', paddingBottom: '8px' }}>
              <input type="checkbox" id={`item-primary-${inventoryItemId}`} checked={isPrimary} onChange={(e) => setIsPrimary(e.target.checked)} />
              <label htmlFor={`item-primary-${inventoryItemId}`} style={{ fontSize: '0.8rem', cursor: 'pointer', userSelect: 'none', color: '#475569' }}>Set Primary</label>
            </div>
      ...
      {/* Gallery Grid */}
      {loading ? (
        <div style={{ padding: '24px', textAlign: 'center', color: '#64748B', fontSize: '0.88rem' }}>Loading physical item photographs...</div>
      ) : images.length === 0 ? (
        <div style={{ padding: '24px', textAlign: 'center', background: '#FFFFFF', borderRadius: '10px', border: '1px solid #E2E8F0', color: '#64748B', fontSize: '0.85rem' }}>
          No physical photographs uploaded for this jewellery item yet.
        </div>
      ) : (
              ...
              <div key={img.id} className="glass-card" style={{ padding: '10px', background: '#FFFFFF', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                {img.isPrimary && (
                  <span className="badge badge-gold" style={{ position: 'absolute', top: '14px', left: '14px', zIndex: 10, padding: '2px 8px', fontSize: '0.68rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Star size={11} fill="#C6A15B" /> Primary Photo
                  </span>
                )}
                <div style={{ height: '140px', width: '100%', borderRadius: '8px', overflow: 'hidden', background: '#F1F5F9', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <img src={fullUrl} alt={img.altText || itemCode} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { (e.target as HTMLElement).style.display = 'none'; }} />
                </div>
                ...
                <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '6px', paddingTop: '6px', borderTop: '1px solid #F1F5F9' }}>
                  <button onClick={() => setPreviewModalUrl(fullUrl)} className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '0.72rem' }} title="Preview Photo">
                    <Eye size={13} />
                  </button>
                  ...
                  {canDelete && (
                    <button onClick={() => handleDelete(img.id)} className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '0.72rem', color: '#DC2626' }} title="Delete Photo">
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to delete this physical photograph? This action cannot be undone."
